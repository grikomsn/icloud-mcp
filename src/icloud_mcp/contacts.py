...
def _get_carddav_session(email: str, password: str) -> tuple:
    ...
    session.headers.update(
        {"Content-Type": "text/xml; charset=utf-8", "User-Agent": "iCloud-MCP/1.0"}
    )
...
def _discover_principal(session: requests.Session, base_url: str) -> str:
    ...
    response = session.request(
        "PROPFIND", base_url, data=propfind_body, headers={"Depth": "0"}
    )
...
def _discover_addressbook_home(session: requests.Session, principal_url: str) -> str:
    ...
    response = session.request(
        "PROPFIND", principal_url, data=propfind_body, headers={"Depth": "0"}
    )
    response.raise_for_status()
...
def _list_addressbooks(
    session: requests.Session, addressbook_home_url: str
) -> List[Dict[str, str]]:
    ...
    response = session.request(
        "PROPFIND", addressbook_home_url, data=propfind_body, headers={"Depth": "1"}
    )
    response.raise_for_status()
    ...
    for response_elem in root.findall(".//d:response", ns):
        ...
        if (
            ...
            and resourcetype_elem.find("card:addressbook", ns) is not None
        ):
            displayname_elem = response_elem.find(".//d:displayname", ns)

            addressbook = {
                "url": (
                    urljoin(addressbook_home_url, href_elem.text)
...
def _fetch_all_vcards(
    session: requests.Session, addressbook_url: str
) -> List[Dict[str, Any]]:
    ...
    try:
        response = session.request(
            "REPORT", addressbook_url, data=query_body, headers={"Depth": "1"}
        )
        response.raise_for_status()
        ...
        for response_elem in root.findall(".//d:response", ns):
            ...
            if vcard_data_elem is not None and vcard_data_elem.text:
                vcards.append(
                    {
                        "url": (
                            urljoin(addressbook_url, href_elem.text)
                            if href_elem is not None
                            else ""
                        ),
...
async def search_contacts(context: Context, query: str) -> List[Dict[str, Any]]:
    """
